const adminDashboard = {
  ...
  showOverview() {
    ...
    const totalFarmers = data.users.filter(u => u.role === 'farmer').length;
        ...
        <table class="table">
          ...
          <tbody>
            ${farmers.map(farmer => {
              ...
              const revenue = farmerOrders.reduce((sum, order) => sum + order.total, 0);
                ...
                <tr>
                  ...
                  <td>
                    <div class="action-buttons">
                      ...
                      <button class="btn-warning btn-sm" onclick="adminDashboard.approveFarmer(${farmer.id})">Approve</button>
                    </div>
                  </td>
                </tr>
              `;
  ...
  showProducts() {
    ...
    const filterProducts = debounce(() => {
      ...
      let filtered = products.filter(product => {
        ...
        if (stockStatus === 'in-stock') matchesStock = product.stock > 10;
        else if (stockStatus === 'low-stock') matchesStock = product.stock <= 10 && product.stock > 0;
  ...
  renderProductRows(products) {
    return products.map(product => `
      <tr>
        ...
        <td>
          <span class="${product.stock < 10 ? (product.stock === 0 ? 'badge badge-danger' : 'badge badge-warning') : ''}">${product.stock}</span>
        </td>
        <td>
        ...
        <td>
          <div class="action-buttons">
            <button class="btn-secondary btn-sm" onclick="adminDashboard.moderateProduct(${product.id})">Moderate</button>
            ...
            <tbody>
              ${data.orders.slice(-10).reverse().map(order => `
                <tr>
                  ...
                  <td>Order #${order.id} - ${formatCurrency(order.total)}</td>
  ...
  approveFarmer(farmerId) {
    showNotification('Farmer approved successfully');
  },

  moderateProduct(productId) {
    showNotification('Product moderation feature coming soon!');
  },
